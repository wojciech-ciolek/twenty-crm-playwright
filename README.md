...
**Network mocking for failure states.** `tests/people/people-network-resilience.spec.ts` uses `page.route()` to intercept the people-list GraphQL query and return a 500, exercising a failure path the real Twenty CRM backend has no way to trigger on demand. This is a different concern from the rest of the suite, which deliberately hits the real API: the goal here isn't "does the integration work" but "what does the UI do when it doesn't." What it does, it turns out, isn't great — a generic Apollo transport error toast that auto-dismisses, then a fallback to the same empty state shown for genuinely zero records — and the tests document that real gap rather than asserting an idealized error UI that doesn't exist.
...
| People | A failed people query surfaces an error toast (mocked via `page.route()`) | `@regression` |
| People | A failed people query falls back to the same empty state as genuinely empty data | `@regression` |